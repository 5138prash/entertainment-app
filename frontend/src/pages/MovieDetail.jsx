...
const Detail = () => {
  // Retrieving the movie ID from the URL parameters
  const { id } = useParams();
  // Fetching movie details using the custom hook
  const { data, isLoading, error } = useGetMovieByIdQuery(id);

  // Safeguard for accessing movie and cast properties
  ...
  return (
    <>
      ...
      {isLoading ? (
        ...
        // Displaying error message if an error occurs during data fetching
          ...
          <div className="col-span-8 flex flex-col gap-4">
            ...
            {movie.vote_average ? (
              <div className="text-[16px] flex items-center gap-5">
                <span>{Number(movie.vote_average / 2).toFixed(2)}</span>
                <span>
                  <StarRating rating={Number(movie.vote_average / 2).toFixed(2)} />
                </span>
              </div>
            ) : 'N/A'}
            ...
            {/* Links to movie's official website and IMDb */}
